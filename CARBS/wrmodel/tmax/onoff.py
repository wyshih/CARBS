...
class Model:
    ...
    def __init__(self, uwr, cpm, appro = None, aid = '215', ctrtrain = False, firstr = False):

        self.dates = dates
        self.hours = hours
        self.bid_floor = 5.#math.exp(math.log(0.012)+5.6)+2
        self.bid_upper = 601.#math.exp(math.log(500.)+5.6)+2
        self.feature_size = pow(2, 7)
    ...
    def loaddata(self, begintime, endtime, aid = None, hour=None, obj = 'daily', gaphour = 1):
        ...
        if aid != None:
            cursor = db.doubleclick.find({'dateTime':{"$gte": time1, '$lt': time2}, "bidResponse.price": {"$gt": 0.}, "winEvent.price": {"$gt":0.}, 'bidResponse.advertiserId':aid})
        else:
            cursor = db.doubleclick.find({'dateTime':{"$gte": time1, '$lt': time2}, "bidResponse.price": {"$gt": 0.}, "winEvent.price": {"$gt":0.}})
        return cursor


    def get_data(self, month, date, hour=None, obj = 'daily', gaphour = 1, aid = None):  
        db = self.client.DoubleClick
        #print month, date
        if obj == 'daily':
            time1 = datetime.datetime.strptime('2016-'+month+'-'+date, '%Y-%m-%d')
            time2 = time1+datetime.timedelta(days=1)#datetime.datetime.strptime('2016-'+month+'-'+str(int(date)+1), '%Y-%m-%d')
        else:
            time1 = datetime.datetime.strptime('2016-'+month+'-'+date+' '+hour, '%Y-%m-%d %H')
            time2 = time1 + datetime.timedelta(hours=gaphour)
            #if hour != '23':
            #    time2 = #datetime.datetime.strptime('2016-'+month+'-'+date+' '+str(int(hour)+1), '%Y-%m-%d %H')
            #else:
            #    time2 = #datetime.datetime.strptime('2016-'+month+'-'+str(int(date)+1)+' 00', '%Y-%m-%d %H')
        #cursor = db.doubleclick.find({"bidResponse.price": {"$gt": 0.}, "winEvent.price": {"$gt":0.}})
        if aid == None:
            cursor = db.doubleclick.find({'dateTime':{"$gte": time1, '$lt': time2}, "bidResponse.price": {"$gt": 0.}, "winEvent.price": {"$gt":0.}})
        else:
            cursor = db.doubleclick.find({'dateTime':{"$gte": time1, '$lt': time2}, "bidResponse.price": {"$gt": 0.}, "winEvent.price": {"$gt":0.}, 'bidResponse.advertiserId':aid})
    ...
    def train_ctrmodel(self, aid, k, clf=None, f=None):
        trainbegintime = datetime.datetime.strptime('2016-10-01', '%Y-%m-%d')
        trainendtime = trainbegintime+datetime.timedelta(days=3)

        validatendtime = trainendtime+datetime.timedelta(days=1)
        testendtime = validatendtime+datetime.timedelta(days=3)
        ...
        if clf == None:
            ...
            para = {'C':[10, 1, .1]}
            clf = GridSearchCV(ctrclf, para, n_jobs = 12)
        ...
        print (clf.best_params_ , ": AUC", clf.best_score_, wpk )
    ...
    def data_prepare(self, each, feature_size, ctrtrain = False):
        x = dict()
        if each['bidRequest'].get('ip'):
            ip = each['bidRequest']['ip'].split('.')
            maskip = ip[0]+'.'+ip[1]+'.'+ip[2]+'.*'
        else:
            maskip = 'null'
        if ctrtrain == False:
            x.setdefault('IP'+"="+maskip, 1)

        if each['bidRequest'].get('region'):
            region = each['bidRequest']['region']
        else:
            region = 'null'
        if ctrtrain == False:
            x.setdefault('Region'+"="+region, 1)

        if each['bidRequest'].get('city'):
            city = each['bidRequest']['city']
        else:
            city = 'null'
        if ctrtrain == False:
            x.setdefault('City'+"="+city, 1)

        if each['bidRequest'].get('userAgency'):
            useragency = each['bidRequest']['userAgency']
        else:
            useragency = 'null'
        x.setdefault('userAgency'+"="+useragency, 1)

        #x.setdefault('AdExchange'+"="+str(each['bidRequest']['adx']), 1)

        if each['bidRequest'].get('url'):
            url = str(each['bidRequest']['url'])
        else:
            url = 'null'
        x.setdefault('URL'+"="+ url, 1)

        if each['bidRequest'].get('domain'):
            domain = each['bidRequest']['domain']
        else:
            domain = 'null'
        x.setdefault('Domain'+"="+domain, 1)

        x.setdefault('AdSlotID'+"="+str(each['bidRequest']['spaceId']), 1)
        #x.setdefault('AdSlotWidth'+"="+str(each['ad_slot_width']), 1)
        #x.setdefault('AdSlotHeight'+"="+str(each['ad_slot_height']), 1)

        #x.setdefault('AdSlotWidth', int(each['bidRequest']['stdAdSize']))
        #x.setdefault('AdSlotHeight', 1)

        x.setdefault('AdSlotVisibility'+"="+str(each['bidRequest']['pos']), 1)
        if each['bidRequest'].get('spaceCat'):
            spacecat = each['bidRequest']['spaceCat']
        else:
            spacecat = 'null'
        x.setdefault('AdSlotFormat'+"="+spacecat, 1)
        x.setdefault('hour'+"="+str(each['dateTime'].hour), 1)
        if  each['bidRequest'].get('doubleClick'):
            for tag in each['bidRequest']['doubleClick']['detectedVerticals']:
                x.setdefault('UserTags'+"="+str(tag['id']), 1)
        else:
            x.setdefault('UserTags'+"="+'null', 1)
        if ctrtrain == False:
    ...
    def traindata_prepare_hash(self, train, feature_size, f = None, allday = False, ctrtrain = False):
        ...
        epswp = list()
        for each in train:
            if each['bidRequest'].get('reqId'):
                audienceid = each['bidRequest']['reqId']
            else:
                audienceid = 'null'
            rid.append(audienceid)
            x = dict()
            if each['bidRequest'].get('ip'):
                ip = each['bidRequest']['ip'].split('.')
                maskip = ip[0]+'.'+ip[1]+'.'+ip[2]+'.*'
            ...
            count += 1
            wprice = math.exp(math.log(float(each['winEvent']['price'])/1.2)+5.6)+2
            bprice = random.uniform((math.exp(math.log(0.012)+5.6)+2), 601.)#0.35*(math.exp(math.log(float(each['bidResponse']['price']))+5.6)+2)
            #train_bid.append(float(each['bidResponse']['price']))
            train_bid.append(bprice)
            ctr.append(each['bidResponse']['mlCtr'])
            if each.get('clickEvent'):
                if each['clickEvent']['click'] == True: 
                    click.append(True)
                else:
                    click.append(False)
            else:
                click.append(False)
            train_data.append(x)
            #if (float(each['bidResponse']['price'])) <= (float(each['winEvent']['price'])/1.2):
            if bprice <= wprice:
                train_label.append(False)
            else:
                if allday:
                    epsmatrix.append(x)
                    epsbid.append(bprice)
                    if self.appro == '3' or self.appro == '1':
                        epswp.append(bprice - float(wprice))
                    else:
                        epswp.append(float(wprice)/bprice)
                train_label.append(True)
            train_win_p.append(wprice)
            #train_win_p.append(float(each['winEvent']['price'])/1.2)
        if len(train_win_p) != 0:
            ...
            if allday == False:
                if ctrtrain == True:
                    ...
                    return train_fdata, train_outbid, train_flabel, train_win_p, train_bid, ctr, click, rid#0#f
            else:
                return train_fdata, train_outbid, train_flabel, train_win_p, train_bid, ctr, click, rid, eps_foutbid, epswp
            #return train_fdata, train_outbid, train_flabel, train_win_p, train_bid, ctr, click, rid#0#f
        else:
    ...
    def given_recommend_price_hash(self, attr, feature_size, coef, w0, winp):
        ...
        '''for each in range(0, len(pred_price)):
    ...
    def given_recommend_price_wr(self, attr, feature_size, coef, w0, winp, ad, pretrain_label, click, ctr = None, dens_ctr = None, th=0.999, epsclf = None, ctrpro = 0., record = False):
            ...
            if count != r:
                ...
                if pretrain_label:
                    ...
                    if self.appro == '3' or self.appro == '4':
                        ...
                        eachprob = ad.cal_winrate(self.winrate_low, self.winrate_up, ad.current_status, coef, self.feature_size, ad.pacing_rate, ad.requests, count, betaX, ad.eps)
                    eachprob = eachprob.real
                    #if ctr != None and norm.cdf(math.log(ctr[count]), loc = dens_ctr[0], scale = dens_ctr[1]) >= th:#0.999:#random.uniform(0, 1) <= ctr[count]:
                    #    eachprob = 0.999
                    eachprob += ctrpro*ctr[count]
                    eachprob = min(0.999, eachprob)
                    #eachprob = min(0.999, eachprob + max(0, (1-float(ctrpro)/ctr[count])))
                price = ((-1)*(math.log(1/eachprob-1)/math.log(math.exp(1)))-betaX)/price_coef
                ad.requests -= 1
                ...
                if price > winp[count] and pretrain_label == False:
                    ...
                    if self.appro == '4':
                        ...
                        #print ad.eps, epsclf.predict(attr[count, :]), (1.-ad.eps)*price, (1.-epsclf.predict(attr[count, :]))*price, price-winp[count]
                    ...
                    if click[count]:
                        ad.clicks += 1
                    #if record and pretrain_label == False:
        ...
        #if pretrain_label == False and record:
        #    bs.append(float(ad.costs))
        #    with open('pickle/ourbudgetspending_215.pickle', 'wb') as a:
        #        pickle.dump(bs, a)
        ...
        if pretrain_label == False:
            if self.appro == '3' or self.appro == '4':
                if epsclf == None:
                    epsclf = SGDRegressor(penalty='l2', alpha = 0.01, fit_intercept=True, n_iter = 1, shuffle = False, warm_start = True, learning_rate = 'constant', eta0 = 0.000000005)#Ridge()
                if self.appro == '3':
                ...
                if self.appro == '4':
                    print ('eps comp, ', np.mean(a5), np.mean(a2), np.mean(a4), 'MSE, ', metrics.mean_squared_error(a5, a2), metrics.mean_squared_error(a5, a4))
                epsclf.partial_fit(eps_foutbid, epswp)
            ad.current_status = ad.budget_next(ad.current_status)
            print ("current rest: ",ad.current_status, 'average winrate', scipy.stats.mstats.gmean(prob), "total cost: ", sum(cpm), 'allocated budget: ', thbudget, "cpm: ", sum(cpm)/len(cpm), "imp: ", len(cpm))
            #if record:
            #    with open('pickle/ours_{0}_{1}_215.pickle'.format(str(ad.budget), str(self.cpm/1000.)), 'wb') as d:
    ...
    def train_model_hash(self, train, label, model = None, init = True, SGD = False):
        ...
        if model == None:
            if SGD:
                clf = SGDClassifier(loss='log', penalty='l2', alpha = 0.01, fit_intercept=True, shuffle = False, warm_start = True, learning_rate = 'constant', eta0 = 0.000000005)
                print ("SGD")
            else:
